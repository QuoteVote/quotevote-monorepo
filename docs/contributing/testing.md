...
The server-side application uses both **Jest** and **Mocha** for its tests, which are located in the `app/tests` directory.

It appears that both frameworks are used to run the same test files, which is an unusual setup. Contributors should be aware of this.

- **Mocha** is used for unit tests. To run them, use the `unittest` script.
- **Jest** is used for integration tests. To run them, use the `test` script.
